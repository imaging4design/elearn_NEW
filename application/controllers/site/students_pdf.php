...
{	
	...
	function __construct()
	{
		...
		$this->load->model('site/studentspdf_model');
	...
	function results_PDF() {
		...
		$this->form_validation->set_rules('studentID', 'Student', 'trim|required');
		...
		if($this->form_validation->run() == TRUE && $this->input->post('token') == $this->session->userdata('token')) 
		{
			...
			/*******************************************************************************************************/
			if($query = $this->studentspdf_model->getstudent_results())
			{
				$students = $query;
			}

			// Gets the Class Name to display at the head of the report
			if($query = $this->studentspdf_model->student_name())
			{
				$student = $query;
			}
		...
		else
		{
			...
			redirect('results/view_progress');
		}
