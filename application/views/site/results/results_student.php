	...
	<div class="row">
		<div class="col-lg-12">
			...
			if( isset($results) && $this->input->post('token'))
			{
				...
				echo '<h5>Last Test Date: <strong>' . $results->test_date . '</strong></h5>';
				...
				echo '<ul id="chart">';
					...
					for ($i = 0, $num = count($month); $i < $num; $i++):
						...
						{
							echo 	'<li>' . $month[$i] . ' ' . date('Y') . ' | <span class="text-red"><strong>' . $average_score_percent . '</strong>%</span> | Tests: <strong>' . $results->$n_month[$i] . '</strong></li>';
			...
			<h3>RESULTS SUMMARY <span class="text-red">(Information):</span></h3>
			...
			<p><span class="text-red"><strong>NOTE:</strong></span> For progress results to be shown, you must complete at least five multi-choice tests (answering a minimum of 10 questions) in each topic.</p>
